...
if (!Room.prototype.__enhancementsLoaded) {
  ...
  Room.prototype.getBestStorageTarget = function (amount, resourceType) {
    if (this.storage && this.terminal) {
      ...
      if (!resourceType) {
        if (
          _.sum(this.storage.store) / this.storage.storeCapacity <
          _.sum(this.terminal.store) / this.terminal.storeCapacity
        ) {
          return this.storage;
        }
      ...
      if (
        storageFree >= amount &&
        terminalFree >= amount &&
        (this.storage.store[resourceType] || 0) / storageFree < (this.terminal.store[resourceType] || 0) / terminalFree
      ) {
        return this.storage;
      }
  ...
  Room.prototype.getBestStorageSource = function (resourceType) {
    if (this.storage && this.terminal) {
      ...
      if (
        (this.storage.store[resourceType] || 0) / this.storage.storeCapacity <
        this.terminal.store[resourceType] / this.terminal.storeCapacity
      ) {
