...
export default class InterShardProcess extends Process {
   ...
   */
  public addShardData(shardName, shardMemory) {
    ...
    if (shardMemory.info) {
      ...
      this._shardData[shardName].neededCpu =
        1 + this._shardData[shardName].rooms + this._shardData[shardName].creeps / 10;
   ...
   */
  public findClosestPortalToSpawnTo(roomName, targetShard) {
    let bestPortal;
    _.each(this.memory.portals, (portals, shardName) => {
