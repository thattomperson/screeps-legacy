...
export default class ResourcesProcess extends Process {
  ...
  public run() {
    ...
    while (best) {
      ...
      if (room.isEvacuating() && terminal.store[best.resourceType] && terminal.store[best.resourceType] > 5000) {
        ...
        hivemind
          .log("trade")
          .info("evacuating", amount, best.resourceType, "from", best.source, "to", best.target, ":", result);
        ...
        room.isEvacuating() &&
        room.storage &&
        !room.storage[best.resourceType] &&
        terminal.store[best.resourceType]
      ) {
        ...
        hivemind
          .log("trade")
          .info("Preparing 5000", best.resourceType, "for transport from", best.source, "to", best.target);
   ...
   */
  public getAvailableTransportRoutes() {
    const options = [];
      ...
      for (const resourceType of _.keys(roomState.state)) {
        ...
        _.each(rooms, (roomState2, roomName2) => {
          ...
          const option = {
            priority: 3,
            weight:
              (roomState.totalResources[resourceType] - roomState2.totalResources[resourceType]) / 100000 -
              Game.map.getRoomLinearDistance(roomName, roomName2),
            resourceType,
            source: roomName,
            target: roomName2
          };
