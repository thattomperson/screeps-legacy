...
export default class TradeProcess extends Process {
   ...
   */
  public instaSellResources(resourceType, rooms) {
    ...
    hivemind
      .log("trade", roomName)
      .debug("Could sell", resourceType, "for", bestOrder.price, "- we want at least", minPrice);
    ...
    if (transactionCost > room.terminal.store.energy) {
      if (room.memory.fillTerminal) {
        hivemind
          .log("trade", roomName)
          .info("Busy, can't prepare", transactionCost, "energy for selling", amount, resourceType);
      } else {
   ...
   *   Resource states for rooms to check, keyed by room name.
   */
  public instaBuyResources(resourceType, rooms) {
    ...
    hivemind
      .log("trade", roomName)
      .debug("Could buy", resourceType, "for", bestOrder.price, "- we want to spend at most", maxPrice);
    ...
    if (isIntershardResource) {
      hivemind
        .log("trade", roomName)
        .info("Buying", amount, resourceType, "from", bestOrder.roomName, "for", bestOrder.price, "credits each.");
    } else {
      const transactionCost = Game.market.calcTransactionCost(amount, roomName, bestOrder.roomName);
      ...
      if (transactionCost > room.terminal.store.energy) {
        if (room.memory.fillTerminal) {
          hivemind
            .log("trade", roomName)
            .info("Busy, can't prepare", transactionCost, "energy for buying", amount, resourceType);
        } else {
          room.prepareForTrading(RESOURCE_ENERGY, transactionCost);
          hivemind
            .log("trade", roomName)
   ...
   */
  public tryBuyResources(resourceType, rooms, ignoreOtherRooms) {
    const isIntershardResource = INTERSHARD_RESOURCES.indexOf(resourceType) !== -1;

    if (
      _.filter(Game.market.orders, order => {
    ...
    hivemind
      .log("trade", roomName)
      .debug("Could offer to buy", resourceType, "for", offerPrice, "- we want to spend at most", maxPrice);
   ...
   *   Resource states for rooms to check, keyed by room name.
   */
  public trySellResources(resourceType, rooms) {
    if (
      _.filter(Game.market.orders, order => order.type === ORDER_SELL && order.resourceType === resourceType).length > 0
    ) {
      return;
    }
    ...
    if (bestSellOrder) {
      ...
      offerPrice = Math.min(
        Math.max(offerPrice, bestSellOrder.price * 0.99),
        history.average * 1.5 + history.stdDev * 2
      );
    } else {
    ...
    hivemind
      .log("trade", roomName)
      .debug("Could offer to sell", resourceType, "for", offerPrice, "- we want at least", minPrice);
   ...
   */
  public getLowestResourceState(resourceType, rooms) {
    let minAmount;
    let bestRoom;
   ...
   */
  public getHighestResourceState(resourceType, rooms) {
    let maxAmount;
    let bestRoom;
    _.each(rooms, (roomState, roomName) => {
      if (!roomState.canTrade) return;
   ...
   */
  public findBestBuyOrder(resourceType, roomName) {
    ...
    _.each(orders, order => {
      ...
      const transactionCost =
        INTERSHARD_RESOURCES.indexOf(resourceType) === -1
          ? Game.market.calcTransactionCost(1000, roomName, order.roomName)
          : 0;
   ...
   *   The order as returned from Game.market.
   */
  public findBestSellOrder(resourceType, roomName) {
  ...
  public removeOldTrades() {
    _.each(Game.market.orders, order => {
      ...
      if (age > 100000 || order.remainingAmount === 0) {
        ...
        hivemind
          .log("trade", order.roomName)
          .debug(
            "Cancelling old trade",
            order.type + "ing",
   ...
   */
  public getResourceTier(resourceType) {
   ...
   */
  public getMaxOrderAmount(resourceType) {
    const history = this.getPriceData(resourceType);
    if (!history) return 0;
   ...
   */
  public getPriceData(resourceType) {
   ...
   */
  public calculateWorth(resourceType) {
