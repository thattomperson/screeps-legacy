...
export default class RemoteMiningOperation extends Operation {
   ...
   */
  public getMiningLocationsByRoom() {
   ...
   */
  public getSourceRoom(sourceLocation) {
  ...
  public hasReservation() {
    ...
    if (room)
      return (
        ...
        room.controller.reservation.username === utilities.getUsername() &&
        room.controller.reservation.ticksToEnd >= CONTROLLER_RESERVE_MAX * 0.1
      );
    ...
    return (
      reservation &&
      reservation.username === utilities.getUsername() &&
      reservation.ticksToEnd >= CONTROLLER_RESERVE_MAX * 0.1
    );
  }
   ...
   */
  public getContainer(sourceLocation) {
   ...
   */
  public hasContainer(sourceLocation) {
   ...
   */
  public getContainerPosition(sourceLocation) {
    const paths = this.getPaths();
   ...
   */
  public shouldSpawnHaulers(sourceLocation) {
   ...
   */
  public getHarvesterSize(sourceLocation) {
    ...
    return paths[sourceLocation] && paths[sourceLocation].accessible
      ? paths[sourceLocation].requiredWorkParts * multiplier
      : 0;
  }
   ...
   */
  public getHaulerSize(sourceLocation) {
    const paths = this.getPaths();
  ...
  public getDismantlePositions(sourceLocation) {
    if (!hivemind.segmentMemory.isReady()) return [];
