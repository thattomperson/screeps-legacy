...
export default class RoomIntel {
  ...
  public constructor(roomName: string) {
    ...
    if (!global.hivemind.segmentMemory.has(key)) {
      global.hivemind.segmentMemory.set(key, {});
    }

    this.memory = global.hivemind.segmentMemory.get(key);
  }
  ...
  public gatherIntel() {
    ...
    hivemind
      .log("intel", room.name)
      .debug("Gathering intel after", intel.lastScan ? Game.time - intel.lastScan : "infinite", "ticks.");
   ...
   */
  public gatherControllerIntel(room) {
    ...
    this.memory.reservation = room.controller
   ...
   */
  public gatherResourceIntel(room) {
    // Check sources.
   ...
   */
  public gatherPowerIntel(powerBanks) {
   ...
   *   An array containing all power banks for the room.
   */
  public gatherPortalIntel(portals) {
   ...
   */
  public gatherStructureIntel(structures, structureType) {
   ...
   */
  public gatherAbandonedResourcesIntel(structures, ruins) {
   ...
   *   An object containing Arrays of structures, keyed by structure type.
   */
  public gatherInvaderIntel(structures) {
   ...
   */
  public generateCostMatrix(structures, constructionSites) {
  ...
  public isClaimed() {
    ...
    if (
      ...
      this.memory.reservation.username &&
      this.memory.reservation.username !== utilities.getUsername()
    )
      return true;
   ...
   */
  public getStructures(structureType) {
   ...
   */
  public calculateAdjacentRoomSafety(options) {
   ...
   */
  public addAdjacentRoomToCheck(roomName, openList, base) {
    if (this.otherUnsafeRooms.indexOf(roomName) === -1) {
      if (Game.rooms[roomName] && Game.rooms[roomName].isMine()) {
        ...
        if (
          Game.rooms[roomName].controller.level >= Math.min(5, this.getRcl() - 1) &&
          !Game.rooms[roomName].isEvacuating()
        )
          return;
   ...
   */
  public handleAdjacentRoom(roomData, openList, closedList) {
